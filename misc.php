...
if($mybb->input['action'] == "dstswitch" && $mybb->request_method == "post" && $mybb->user['uid'] > 0) {
...
else if($mybb->input['action'] == "markread") {
	...
	if(isset($mybb->input['fid'])) {
		...
		include_once MYBB_ROOT."/inc/functions_indicators.php";
...
else if($mybb->input['action'] == "clearpass") {
...
else if($mybb->input['action'] == "rules") {
	if(isset($mybb->input['fid'])) {
...
else if($mybb->input['action'] == "do_helpsearch" && $mybb->request_method == "post") {
...
else if($mybb->input['action'] == "helpresults") {
	if($mybb->settings['helpsearch'] != 1) {
		error($lang->error_helpsearchdisabled);
	}
...
else if($mybb->input['action'] == "buddypopup") {
...
else if($mybb->input['action'] == "whoposted") {
	$numposts  = 0;
	$altbg     = alt_trow();
	$whoposted = '';
...
else if($mybb->input['action'] == "smilies") {
	$smilies = '';
	if(!empty($mybb->input['popup']) && !empty($mybb->input['editor'])) { // make small popup list of smilies
...
else if($mybb->input['action'] == "syndication") {
...
else if($mybb->input['action'] == "clearcookies") {
