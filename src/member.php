...
/**
 * Member page used for various things.
 *
 * @package MyBB 1.8
 * @author  MyBB Group
...
if ($mybb->input['action'] == "do_register" && $mybb->request_method == "post") {
	...
	if ($mybb->settings['enablestopforumspam_on_register']) {
		include_once MYBB_ROOT . '/inc/class_stopforumspamchecker.php';
		...
		try {
			if ($stop_forum_spam_checker->is_user_a_spammer($user['username'], $user['email'], get_ip())) {
				error($lang->sprintf(
					$lang->error_stop_forum_spam_spammer,
	...
	if ($mybb->settings['captchaimage']) {
		include_once MYBB_ROOT . 'inc/class_captcha.php';
		$captcha = new captcha;
			...
			if (!empty($admingroups)) {
				...
				while ($recipient = $db->fetch_array($q)) {
					...
					if (!$is_super_admin) {
						...
						include_once MYBB_ROOT . $mybb->config['admin_dir'] . "/inc/functions.php";

						// Verify if we have permissions to access user-users
						include_once MYBB_ROOT . $mybb->config['admin_dir'] . "/modules/user/module_meta.php";
...
if ($mybb->input['action'] == "login") {
	...
	if ($mybb->settings['captchaimage'] && $do_captcha == TRUE) {
		...
		} else if (in_array($login_captcha->type, array(NOCAPTCHA_RECAPTCHA, RECAPTCHA_INVISIBLE, RECAPTCHA_V3))) {
			$login_captcha->build_recaptcha();
		} else if (in_array($login_captcha->type, array(HCAPTCHA, HCAPTCHA_INVISIBLE))) {
...
if ($mybb->input['action'] == "profile") {
	...
	if ($memprofile['away'] == 1 && $mybb->settings['allowaway'] != 0) {
		...
		if ($memprofile['returndate'] == '') {
			...
			if ($returnhome[2] >= 2038) {
				include_once MYBB_ROOT . "inc/functions_time.php";
	...
	} else if (trim($memperms['usertitle']) != '') {
	...
	if (!empty($last_seen)) {
		...
		if ($memprofile['invisible'] == 1 && $mybb->usergroup['canviewwolinvis'] != 1 && $memprofile['uid'] != $mybb->user['uid']) {
			...
			if (!empty($session)) {
				...
				$lang->load("online");
				include_once MYBB_ROOT . "inc/functions_online.php";
	...
	if ($mybb->usergroup['canmodcp'] == 1 || $can_purge_spammer) {
		...
		} else if (modcp_can_manage_user($memprofile['uid']) && $mybb->usergroup['canbanusers'] == 1) {
...
if ($mybb->input['action'] == "do_emailuser" && $mybb->request_method == "post") {
	...
	if ($mybb->usergroup['emailfloodtime'] > 0) {
		...
		if ($last_email['mid']) {
			...
			} else if ($remaining_time > 60 && $remaining_time < 120) {
