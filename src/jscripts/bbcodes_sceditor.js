$(function ($) {
	...
	var mybbCmd = {
		...
		video: {
			Dailymotion: {
				...
				html:
					'<iframe frameborder="0" width="480" height="270" src="{url}" data-mybb-vt="{type}" data-mybb-vsrc="{src}"></iframe>',
			},
			Facebook: {
				match: /facebook\.com\/(?:photo.php\?v=|video\/video.php\?v=|video\/embed\?video_id=|v\/?)(\d+)/,
				url: "https://www.facebook.com/video/embed?video_id=",
				html:
					'<iframe src="{url}" width="625" height="350" frameborder="0" data-mybb-vt="{type}" data-mybb-vsrc="{src}"></iframe>',
			},
			Liveleak: {
				match: /liveleak\.com\/(?:view\?[a-z]=)([^\/]+)/,
				url: "http://www.liveleak.com/ll_embed?i=",
				html:
					'<iframe width="500" height="300" src="{url}" frameborder="0" data-mybb-vt="{type}" data-mybb-vsrc="{src}"></iframe>',
			},
			MetaCafe: {
				match: /metacafe\.com\/watch\/([^\/]+)/,
				url: "http://www.metacafe.com/embed/",
				html:
					'<iframe src="{url}" width="440" height="248" frameborder=0 data-mybb-vt="{type}" data-mybb-vsrc="{src}"></iframe>',
			},
			Mixer: {
				match: /mixer\.com\/([^\/]+)/,
				url: "//mixer.com/embed/player/",
				html:
					'<iframe allowfullscreen="true" src="{url}" width="620" height="349" frameborder="0" data-mybb-vt="{type}" data-mybb-vsrc="{src}"></iframe>',
			},
			Vimeo: {
				match: /vimeo.com\/(\d+)($|\/)/,
				url: "//player.vimeo.com/video/",
				html:
					'<iframe src="{url}" width="500" height="281" frameborder="0" data-mybb-vt="{type}" data-mybb-vsrc="{src}"></iframe>',
			},
			Youtube: {
				match: /(?:v=|v\/|embed\/|youtu\.be\/)(.{11})/,
				url: "//www.youtube-nocookie.com/embed/",
				html:
					'<iframe width="560" height="315" src="{url}" frameborder="0" data-mybb-vt="{type}" data-mybb-vsrc="{src}"></iframe>',
			},
			Twitch: {
				match: /twitch\.tv\/(?:[\w+_-]+)\/v\/(\d+)/,
				url: "//player.twitch.tv/?video=v",
				html:
					'<iframe src="{url}" frameborder="0" scrolling="no" height="378" width="620" data-mybb-vt="{type}" data-mybb-vsrc="{src}"></iframe>',
			},
		},
	};
	...
	$.sceditor.formats.bbcode
		.set("list", {
			html: function (element, attrs, content) {
				...
				if (attrs.defaultattr === "a") type = 'ol type="a"';
	...
	$.sceditor.formats.bbcode.set("font", {
		...
		html: function (token, attrs, content) {
			if (
				typeof attrs.defaultattr == "string" &&
				attrs.defaultattr != "" &&
	...
	$.sceditor.formats.bbcode.set("img", {
		format: function (element, content) {
			if ($(element).data("sceditor-emoticon")) return content;
