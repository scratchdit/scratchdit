...
class UserDataHandler extends DataHandler
{
	...
	function verify_birthday()
	{
		...
		if ($mybb->settings['coppa'] == "enabled" && ($birthday['year'] == 0 || !$birthday['year'])) {
			...
			return FALSE;
		} else if (($mybb->settings['coppa'] == "deny" && $birthday['year'] > (date("Y") - 13)) && !is_moderator()) {
	...
	function verify_profile_fields()
	{
		...
		if (is_array($pfcache)) {
			...
			foreach ($pfcache as $profilefield) {
				...
				} else if (($type == "multiselect" || $type == "checkbox") && $profile_fields[$field] == "" && $profilefield['required'] == 1 && !defined('IN_ADMINCP') && THIS_SCRIPT != "modcp.php") {
					$this->set_error('missing_required_profile_field', array($profilefield['name']));
				}
	...
	function delete_content($delete_uids = FALSE)
	{
		...
		include_once MYBB_ROOT . 'inc/functions_upload.php';
	...
	function delete_posts($delete_uids = FALSE)
	{
		...
		include_once MYBB_ROOT . 'inc/class_moderation.php';
