...
{
	global $db, $lang, $theme, $templates, $plugins, $mybb, $debug, $templatecache, $templatelist, $maintimer, $globaltime, $parsetime;
...
function forum_password_validated($forum, $ignore_empty = FALSE, $check_parents = FALSE)
{
	...
	return ($ignore_empty && $forum['password'] === '') || (isset($mybb->cookies['forumpass'][$forum['fid']]) &&
		my_hash_equals(
...
function build_highlight_array($terms)
{
	...
	foreach ($words as $word) {
		...
		$replacement = "<span class=\"highlight\" style=\"padding-left: 0; padding-right: 0;\">$1</span>";
...
function trim_blank_chrs($string, $charlist = "")
{
	$hex_chrs = array(
		...
		0xE2 => array(
			0x80 => array(
				0x80 => 1, 0x81 => 1, 0x82 => 1, 0x83 => 1, 0x84 => 1, 0x85 => 1, 0x86 => 1, 0x87 => 1, 0x88 => 1, 0x89 => 1, 0x8A => 1, 0x8B => 1, 0x8C => 1, 0x8D => 1, 0x8E => 1, 0x8F => 1, // \x{2000} - \x{200F}
				0xA8 => 1, 0xA9 => 1, 0xAA => 1, 0xAB => 1, 0xAC => 1, 0xAD => 1, 0xAE => 1, 0xAF => 1
			), // \x{2028} - \x{202F}
			0x81 => array(0x9F => 1)
		), // \x{205F}
		0xE3 => array(
			0x80 => array(0x80 => 1), // \x{3000}
			0x85 => array(0xA4 => 1)
		), // \x{3164}
		0xEF => array(
			0xBB => array(0xBF => 1), // \x{FEFF}
			0xBE => array(0xA0 => 1), // \x{FFA0}
			0xBF => array(0xB9 => 1, 0xBA => 1, 0xBB => 1)
		), // \x{FFF9} - \x{FFFB}
	);
