...
class MyBB
{
	...
	function __construct()
	{
		...
		if (isset($this->input['action']) && $this->input['action'] == "mybb_logo") {
			include_once dirname(__FILE__) . "/mybb_group.php";
			output_logo();
		}
	...
	function get_input($name, $type = self::INPUT_STRING)
	{
		switch ($type) {
			...
			case self::INPUT_FLOAT:
				if (!isset($this->input[$name]) || !is_numeric($this->input[$name])) {
					return 0.0;
				}
				return (float)$this->input[$name];
			case self::INPUT_BOOL:
	...
	{
		echo "<!-- get_asset_url($path, $use_cdn) -->";
	...
	function trigger_generic_error($code)
	{
		...
		switch ($code) {
			...
			case "board_not_upgraded":
				...
				break;
			case "sql_load_error":
				$message    = "MyBB was unable to load the SQL extension. Please contact the MyBB Group for support. <a href=\"//mybb.com\">MyBB Website</a>";
			...
			case "redis_load_error":
				...
				break;
			default:
				$message    = "MyBB has experienced an internal error. Please contact the MyBB Group for support. <a href=\"//mybb.com\">MyBB Website</a>";
